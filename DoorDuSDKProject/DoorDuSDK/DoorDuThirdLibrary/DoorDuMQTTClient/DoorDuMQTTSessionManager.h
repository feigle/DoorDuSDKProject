 ...
 */
@protocol DoorDuMQTTSessionManagerDelegate <NSObject>

/**
 Enumeration of DoorDuMQTTSessionManagerState values
 */
typedef NS_ENUM(int, DoorDuMQTTSessionManagerState) {
    DoorDuMQTTSessionManagerStateStarting,
    DoorDuMQTTSessionManagerStateConnecting,
    DoorDuMQTTSessionManagerStateError,
    DoorDuMQTTSessionManagerStateConnected,
    DoorDuMQTTSessionManagerStateClosing,
    DoorDuMQTTSessionManagerStateClosed
};
...
/** SessionManager handles the DoorDuMQTT session for your application
 */
@interface DoorDuMQTTSessionManager : NSObject <DoorDuMQTTSessionDelegate>

/** the delegate receiving incoming messages
 */
@property (weak, nonatomic) id<DoorDuMQTTSessionManagerDelegate> delegate;

/** subscriptions is a dictionary of NSNumber instances indicating the DoorDuMQTTQoSLevel.
...
/** effectiveSubscriptions s a dictionary of NSNumber instances indicating the granted DoorDuMQTTQoSLevel, or 0x80 for subscription failure.
 *  The keys are topic filters.
 *  effectiveSubscriptions is observable and is updated everytime subscriptions change
 *  @code
 ...
 DoorDuMQTTSessionManager *manager = [[DoorDuMQTTSessionManager alloc] init];
 ...
 DoorDuMQTTSessionManager *manager = (DoorDuMQTTSessionManager *)object;
 ...
 */
@property (nonatomic, readonly) DoorDuMQTTSessionManagerState state;

/** SessionManager last error code when state equals DoorDuMQTTSessionManagerStateError
 */
@property (nonatomic, readonly) NSError *lastErrorCode;

/** initWithPersistence sets the DoorDuMQTTPersistence properties other than default
 ...
 * @return the initialized DoorDuMQTTSessionManager object
 */
...
/** Connects to the DoorDuMQTT broker and stores the parameters for subsequent reconnects
 ...
 * @param keepalive The Keep Alive is a time interval measured in seconds. The DoorDuMQTTClient ensures that the interval between Control Packets being sent does not exceed the Keep Alive value. In the  absence of sending any other Control Packets, the Client sends a PINGREQ Packet.
...
/** Re-Connects to the DoorDuMQTT broker using the parameters for given in the connectTo method
 */
- (void)connectToLast;
...
/** Disconnects gracefully from the DoorDuMQTT broker
 */
- (void)disconnect;

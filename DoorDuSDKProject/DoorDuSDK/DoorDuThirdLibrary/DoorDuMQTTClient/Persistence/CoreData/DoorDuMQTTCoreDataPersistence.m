...
@interface DoorDuMQTTCoreDataFlow ()

- (DoorDuMQTTCoreDataFlow *)initWithContext:(NSManagedObjectContext *)context andObject:(id<DoorDuMQTTFlow>)object;
@property NSManagedObjectContext *context;
@property id<DoorDuMQTTFlow> object;

@end

@implementation DoorDuMQTTCoreDataFlow

@synthesize context;
@synthesize object;

- (DoorDuMQTTCoreDataFlow *)initWithContext:(NSManagedObjectContext *)c andObject:(id<DoorDuMQTTFlow>)o
{
    self = [super init];
    self.context = c;
    self.object = o;
...
@implementation DoorDuMQTTCoreDataPersistence
@synthesize persistent;
@synthesize maxSize;
@synthesize maxMessages;
@synthesize maxWindowSize;

- (DoorDuMQTTCoreDataPersistence *)init {
    self = [super init];
...
- (NSUInteger)windowSize:(NSString *)clientId {
    ...
    NSArray *flows = [self allFlowsforClientId:clientId
                                  incomingFlag:NO];
    for (DoorDuMQTTCoreDataFlow *flow in flows) {
        if ([flow.commandType unsignedIntegerValue] != DoorDuMQTT_None) {
            windowSize++;
        }
    }
    return windowSize;
...
- (DoorDuMQTTCoreDataFlow *)storeMessageForClientId:(NSString *)clientId
    ...
    if (([self allFlowsforClientId:clientId incomingFlag:incomingFlag].count <= self.maxMessages) &&
        ...
        DoorDuMQTTCoreDataFlow *flow = [self createFlowforClientId:clientId
...
- (void)deleteFlow:(DoorDuMQTTCoreDataFlow *)flow {
    if ([NSThread isMainThread]) {
...
- (void)deleteAllFlowsForClientId:(NSString *)clientId {
    if ([NSThread isMainThread]) {
        for (DoorDuMQTTCoreDataFlow *flow in [self allFlowsforClientId:clientId incomingFlag:TRUE]) {
            [self.managedObjectContext deleteObject:(NSManagedObject *)flow.object];
        }
        for (DoorDuMQTTCoreDataFlow *flow in [self allFlowsforClientId:clientId incomingFlag:FALSE]) {
            [self.managedObjectContext deleteObject:(NSManagedObject *)flow.object];
        }
    } else {
        [self.managedObjectContext performBlockAndWait:^{
...
- (void)internalSync {
    if (self.managedObjectContext.hasChanges) {
        NSLog(@"[MQTTPersistence] pre-sync: i%lu u%lu d%lu",
        ...
        NSLog(@"[MQTTPersistence] postsync: i%lu u%lu d%lu",
                     (unsigned long)self.managedObjectContext.insertedObjects.count,
                     (unsigned long)self.managedObjectContext.updatedObjects.count,
                     (unsigned long)self.managedObjectContext.deletedObjects.count
                     );
...
- (void)internalParentSync {
    if (self.managedObjectContext.parentContext && self.managedObjectContext.parentContext.hasChanges) {
        ...
        if (![self.managedObjectContext.parentContext save:&error]) {
            NSLog(@"[MQTTPersistence] parentContext sync error %@", error);
        }
    }
}
...
- (NSArray *)allFlowsforClientId:(NSString *)clientId
    ...
    if ([NSThread isMainThread]) {
        ...
        if (!rows) {
            NSLog(@"[MQTTPersistence] allFlowsforClientId %@", error);
        }
    } else {
        [self.managedObjectContext performBlockAndWait:^{
    ...
    for (id<DoorDuMQTTFlow>row in rows) {
        [flows addObject:[[DoorDuMQTTCoreDataFlow alloc] initWithContext:self.managedObjectContext andObject:row]];
    }
    return flows;
}

- (DoorDuMQTTCoreDataFlow *)flowforClientId:(NSString *)clientId
                         incomingFlag:(BOOL)incomingFlag
                            messageId:(UInt16)messageId {
    __block DoorDuMQTTCoreDataFlow *flow = nil;
...
- (DoorDuMQTTCoreDataFlow *)internalFlowForClientId:(NSString *)clientId
                                 incomingFlag:(BOOL)incomingFlag
                                    messageId:(UInt16)messageId {
    DoorDuMQTTCoreDataFlow *flow = nil;
    ...
    rows = [self.managedObjectContext executeFetchRequest:fetchRequest error:&error];
    if (!rows) {
        NSLog(@"[MQTTPersistence] flowForClientId %@", error);
    } else {
        if ([rows count]) {
            flow = [[DoorDuMQTTCoreDataFlow alloc] initWithContext:self.managedObjectContext andObject:[rows lastObject]];
        }
    }
    return flow;
}

- (DoorDuMQTTCoreDataFlow *)createFlowforClientId:(NSString *)clientId
                               incomingFlag:(BOOL)incomingFlag
                                  messageId:(UInt16)messageId {
    DoorDuMQTTCoreDataFlow *flow = (DoorDuMQTTCoreDataFlow *)[self flowforClientId:clientId
                                                          incomingFlag:incomingFlag
                                                             messageId:messageId];
    if (!flow) {
        __block id<DoorDuMQTTFlow> row;
        if ([NSThread isMainThread]) {
        ...
        flow = [[DoorDuMQTTCoreDataFlow alloc] initWithContext:self.managedObjectContext andObject:row];
    }
...
- (NSPersistentStoreCoordinator *)persistentStoreCoordinator
{
    @synchronized (lock) {
        ...
        NSLog(@"[MQTTPersistence] Persistent store: %@", persistentStoreURL.path);
        ...
        if (![persistentStoreCoordinator addPersistentStoreWithType:self.persistent ? NSSQLiteStoreType : NSInMemoryStoreType
                                                      configuration:nil
                                                            ...
                                                            options:options
                                                              error:&error]) {
            NSLog(@"[MQTTPersistence] managedObjectContext save: %@", error);
...
- (void)sizes {
    ...
    NSLog(@"[MQTTPersistence] sizes %llu/%llu", fileSize, fileSystemFreeSize);
}
@end

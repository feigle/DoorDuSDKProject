...
@class DoorDuMQTTSession;
@class DoorDuMQTTSSLSecurityPolicy;

/**
 Enumeration of DoorDuMQTTSession states
 */
typedef NS_ENUM(NSInteger, DoorDuMQTTSessionStatus) {
    DoorDuMQTTSessionStatusCreated,
    DoorDuMQTTSessionStatusConnecting,
    DoorDuMQTTSessionStatusConnected,
    DoorDuMQTTSessionStatusDisconnecting,
    DoorDuMQTTSessionStatusClosed,
    DoorDuMQTTSessionStatusError
};

/**
 Enumeration of DoorDuMQTTSession events
 */
typedef NS_ENUM(NSInteger, DoorDuMQTTSessionEvent) {
    DoorDuMQTTSessionEventConnected,
    DoorDuMQTTSessionEventConnectionRefused,
    DoorDuMQTTSessionEventConnectionClosed,
    DoorDuMQTTSessionEventConnectionError,
    DoorDuMQTTSessionEventProtocolError,
    DoorDuMQTTSessionEventConnectionClosedByBroker
};

/**
 The error domain used for all errors created by DoorDuMQTTSession
 */
extern NSString * const DoorDuMQTTSessionErrorDomain;

/**
 The error codes used for all errors created by DoorDuMQTTSession
 */
typedef NS_ENUM(NSInteger, DoorDuMQTTSessionError) {
    DoorDuMQTTSessionErrorConnectionRefused = -8, // Sent if the server closes the connection without sending an appropriate error CONNACK
    DoorDuMQTTSessionErrorIllegalMessageReceived = -7,
    DoorDuMQTTSessionErrorDroppingOutgoingMessage = -6, // For some reason the value is the same as for DoorDuMQTTSessionErrorNoResponse
    DoorDuMQTTSessionErrorNoResponse = -6, // For some reason the value is the same as for DoorDuMQTTSessionErrorDroppingOutgoingMessage
    DoorDuMQTTSessionErrorEncoderNotReady = -5,
    DoorDuMQTTSessionErrorInvalidConnackReceived = -2, // Sent if the message received from server was an invalid connack message
    DoorDuMQTTSessionErrorNoConnackReceived = -1, // Sent if first message received from server was no connack message
    DoorDuMQTTSessionErrorConnackUnacceptableProtocolVersion = 1, // Value as defined by DoorDuMQTT Protocol
    DoorDuMQTTSessionErrorConnackIdentifierRejected = 2, // Value as defined by DoorDuMQTT Protocol
    DoorDuMQTTSessionErrorConnackServeUnavailable = 3, // Value as defined by DoorDuMQTT Protocol
    DoorDuMQTTSessionErrorConnackBadUsernameOrPassword = 4, // Value as defined by DoorDuMQTT Protocol
    DoorDuMQTTSessionErrorConnackNotAuthorized = 5, // Value as defined by DoorDuMQTT Protocol
    DoorDuMQTTSessionErrorConnackReserved = 6, // Should be value 6-255, as defined by DoorDuMQTT Protocol
};

/** Session delegate gives your application control over the DoorDuMQTTSession
 @note all callback methods are optional
 */

@protocol DoorDuMQTTSessionDelegate <NSObject>

@optional

/** gets called when a new message was received
 @param session the DoorDuMQTTSession reporting the new message
 ...
 */
- (BOOL)newMessageWithFeedback:(DoorDuMQTTSession *)session
                          data:(NSData *)data
                       onTopic:(NSString *)topic
                           qos:(DoorDuMQTTQosLevel)qos
                      retained:(BOOL)retained
 ...
 */
- (void)session:(DoorDuMQTTSession*)session newMessage:(NSData*)data onTopic:(NSString*)topic;

/** gets called when a connection is established, closed or a problem occurred
 @param session the DoorDuMQTTSession reporting the event
...
/** for DoorDuMQTTio-OBJC backward compatibility
 ...
 @param eventCode the code of the event
 */
- (void)session:(DoorDuMQTTSession*)session handleEvent:(DoorDuMQTTSessionEvent)eventCode;

/** gets called when a connection has been successfully established
 @param session the DoorDuMQTTSession reporting the connect
...
/** gets called when a connection has been refused
 @param session the DoorDuMQTTSession reporting the refusal
 @param error an optional additional error object with additional information
 */
- (void)connectionRefused:(DoorDuMQTTSession *)session error:(NSError *)error;

/** gets called when a connection has been closed
 @param session the DoorDuMQTTSession reporting the close

 */
- (void)connectionClosed:(DoorDuMQTTSession *)session;

/** gets called when a connection error happened
 @param session the DoorDuMQTTSession reporting the connect error
 @param error an optional additional error object with additional information
 */
- (void)connectionError:(DoorDuMQTTSession *)session error:(NSError *)error;

/** gets called when an DoorDuMQTT protocol error happened
 @param session the DoorDuMQTTSession reporting the protocol error
 @param error an optional additional error object with additional information
 */
- (void)protocolError:(DoorDuMQTTSession *)session error:(NSError *)error;

/** gets called when a published message was actually delivered
 @param session the DoorDuMQTTSession reporting the delivery
...
/** gets called when a subscription is acknowledged by the DoorDuMQTT broker
 @param session the DoorDuMQTTSession reporting the acknowledge
 ...
 */
- (void)subAckReceived:(DoorDuMQTTSession *)session msgID:(UInt16)msgID grantedQoss:(NSArray<NSNumber *> *)qoss;

/** gets called when an unsubscribe is acknowledged by the DoorDuMQTT broker
 @param session the DoorDuMQTTSession reporting the acknowledge
 @param msgID the Message Identifier of the UNSUBSCRIBE message
 */
- (void)unsubAckReceived:(DoorDuMQTTSession *)session msgID:(UInt16)msgID;

/** gets called when a command is sent to the DoorDuMQTT broker
 use this for low level monitoring of the DoorDuMQTT connection
 @param session the DoorDuMQTTSession reporting the sent command
 @param type the DoorDuMQTT command type
 ...
 */
- (void)sending:(DoorDuMQTTSession *)session type:(DoorDuMQTTCommandType)type qos:(DoorDuMQTTQosLevel)qos retained:(BOOL)retained duped:(BOOL)duped mid:(UInt16)mid data:(NSData *)data;

/** gets called when a command is received from the DoorDuMQTT broker
 use this for low level monitoring of the DoorDuMQTT connection
 @param session the DoorDuMQTTSession reporting the received command
 @param type the DoorDuMQTT command type
 @param qos the Quality of Service of the command
 @param retained the retained status of the command
 @param duped the duplication status of the command
 ...
 @param data the payload data of the command if any, might be zero length
 */
- (void)received:(DoorDuMQTTSession *)session type:(DoorDuMQTTCommandType)type qos:(DoorDuMQTTQosLevel)qos retained:(BOOL)retained duped:(BOOL)duped mid:(UInt16)mid data:(NSData *)data;

/** gets called when a command is received from the DoorDuMQTT broker
 use this for low level control of the DoorDuMQTT connection
 @param session the DoorDuMQTTSession reporting the received command
 @param type the DoorDuMQTT command type
 @param qos the Quality of Service of the command
 @param retained the retained status of the command
 ...
 */
- (BOOL)ignoreReceived:(DoorDuMQTTSession *)session type:(DoorDuMQTTCommandType)type qos:(DoorDuMQTTQosLevel)qos retained:(BOOL)retained duped:(BOOL)duped mid:(UInt16)mid data:(NSData *)data;

/** gets called when the content of DoorDuMQTTClients internal buffers change
 use for monitoring the completion of transmitted and received messages
 @param session the DoorDuMQTTSession reporting the change
 @param queued for backward compatibility only: DoorDuMQTTClient does not queue messages anymore except during QoS protocol
 @param flowingIn the number of incoming messages not acknowledged by the DoorDuMQTTClient yet
 @param flowingOut the number of outgoing messages not yet acknowledged by the DoorDuMQTT broker
 */
- (void)buffered:(DoorDuMQTTSession *)session
...
typedef void (^DoorDuMQTTConnectHandler)(NSError *error);
typedef void (^DoorDuMQTTDisconnectHandler)(NSError *error);
typedef void (^DoorDuMQTTSubscribeHandler)(NSError *error, NSArray<NSNumber *> *gQoss);
typedef void (^DoorDuMQTTUnsubscribeHandler)(NSError *error);
typedef void (^DoorDuMQTTPublishHandler)(NSError *error);

/** Session implements the DoorDuMQTT protocol for your application
 *
 */

@interface DoorDuMQTTSession : NSObject

/** set this member variable to receive delegate messages
 @code
 #import "DoorDuMQTTClient.h"
 
 @interface MyClass : NSObject <DoorDuMQTTSessionDelegate>
 ...
 @end
 ...
 ...
 - (void)handleEvent:(DoorDuMQTTSession *)session
        event:(DoorDuMQTTSessionEvent)eventCode
        error:(NSError *)error {
    ...
 }
 - (void)newMessage:(DoorDuMQTTSession *)session
 ...
 */
@property (weak, nonatomic) id<DoorDuMQTTSessionDelegate> delegate;

/** Control DoorDuMQTT persistence by setting the properties of persistence before connecting to an DoorDuMQTT broker.
    The settings are specific to a clientId.
 
    persistence.persistent = YES or NO (default) to establish file or in memory persistence. IMPORTANT: set immediately after creating the DoorDuMQTTSession before calling any other method. Otherwise the default value (NO) will be used
        for this session.
...
*/
@property (strong, nonatomic) id<DoorDuMQTTPersistence> persistence;

/** block called once when connection is established
 */
@property (copy, nonatomic) DoorDuMQTTConnectHandler connectHandler;

/** block called when connection is established
 */
@property (strong) void (^connectionHandler)(DoorDuMQTTSessionEvent event);
...
/** Session status
 */
@property (nonatomic, readonly) DoorDuMQTTSessionStatus status;
 ...
 * The DoorDuMQTTClient ensures that the interval between Control Packets being sent does not exceed
 ...
 * If the Will Flag is set to YES, the Will QoS MUST be a valid DoorDuMQTTQosLevel.
 */
@property (nonatomic) DoorDuMQTTQosLevel willQoS;
 ...
 * MUST publish the Will Message as a non-retained publication [DoorDuMQTT-3.1.2-14].
 * If Will Retain is set to YES, the Server MUST publish the Will Message as a retained publication [DoorDuMQTT-3.1.2-15].
 */
 ...
 */
@property (strong, nonatomic) DoorDuMQTTMessage *connectMessage;

/** the transport provider for DoorDuMQTTClient
 *
 * assign an in instance of a class implementing the DoorDuMQTTTransport protocol e.g.
 * DoorDuMQTTCFSocketTransport before connecting.
 */
@property (strong, nonatomic) id <DoorDuMQTTTransport> transport;
 ...
 *  DoorDuMQTT session parameters asynchronously
 *
...
/** initialises the DoorDuMQTT session with default values
...
/** subscribes to a topic at a specific QoS level
 ...
 @note returns immediately. To check results, register as an DoorDuMQTTSessionDelegate and watch for events.
...
- (UInt16)subscribeToTopic:(NSString *)topic atLevel:(DoorDuMQTTQosLevel)qosLevel;
/** subscribes to a topic at a specific QoS level
...
- (UInt16)subscribeToTopic:(NSString *)topic atLevel:(DoorDuMQTTQosLevel)qosLevel subscribeHandler:(DoorDuMQTTSubscribeHandler)subscribeHandler;
...
/** publishes data on a given topic at a specified QoS level and retain flag
 ...
 @param retainFlag if YES, data is stored on the DoorDuMQTT broker until overwritten by the next publish with retainFlag = YES
...
- (UInt16)publishData:(NSData *)data onTopic:(NSString *)topic retain:(BOOL)retainFlag qos:(DoorDuMQTTQosLevel)qos;
...
- (UInt16)publishData:(NSData *)data onTopic:(NSString *)topic retain:(BOOL)retainFlag qos:(DoorDuMQTTQosLevel)qos publishHandler:(DoorDuMQTTPublishHandler)publishHandler;

/** closes an DoorDuMQTTSession gracefully

...
/** DoorDuMQTTCFSocketTransport
 * implements an DoorDuMQTTTransport on top of CFNetwork
 */
@interface DoorDuMQTTCFSocketTransport : DoorDuMQTTTransport <DoorDuMQTTTransport, DoorDuMQTTCFSocketDecoderDelegate, DoorDuMQTTCFSocketEncoderDelegate>
...
/** reads the content of a PKCS12 file and converts it to an certificates array for initWith...
 ...
 @code
 NSString *path = [[NSBundle bundleForClass:[DoorDuMQTTClientTests class]] pathForResource:@"filename"
 ofType:@"p12"];
 
 NSArray *myCerts = [DoorDuMQTTCFSocketTransport clientCertsFromP12:path passphrase:@"passphrase"];
 if (myCerts) {
 
 self.session = [[DoorDuMQTTSession alloc] init];
 ...
